...
%    	Bug Fix: Fixed issue with default template file name
% NOTES: 1. To take advantage of multi core/proc processing Matlab must must run
...
%%  Initialize Variables
    ...
    global indexR ibi h art flagReady flagMultiCore flagDBtb Ann
    
    rate=1000; %sample rate of ecg
    
    flagReady=false; %flag that disables user controls until ecg is loaded    
    
    %check if db toolbox is installed. If not then all db annotation fxn
    %will be disabled
    flagDBtb=license('test','Database_Toolbox');    
        
    dataSource= 'ann_db';%specifies the datasourse to use for annotations. 
                         %Make sure this matches the datasource that is defined in Windows
...
%% Defaults for gui
    ...
    %Beat detection
     def_template=fullfile(def_dir,'sample_template.mat');
     def_tempUT='0.4';   %upper thresh for template matching
...
%% Create ECG Filter Controls
   ...
   %% Create Results
    ...
    listFilter_Callback; %simulate filter list change to ensure options are displayed
    
    %warn user if db toolbox is not installed
    if ~flagDBtb
        warndlg(['Database Toolbox required for using annotions. All annotation', ...
    ...
    %check for availability of using parallel computing for template matching                        
    if license('test','Distrib_Computing_Toolbox') %check for parallel computing license            
...
%%  Callbacks
    ...
    function lstFilesClk_Callback(hObject, eventdata)
        ...
        if ~flagDBtb; return; end; %if db toolbox doesn't exist exit function
...
%         try
            ...
            if ~isequal(fName,0) || ~isequal(fPath,0)
                ...
                switch extn
                    case '.ibi' %Text
                        tmp=ibi(x1:x2,:);                                              
                        dlmwrite(fullfile(fPath,fName), tmp, 'delimiter', ',' ...
                            ,'precision', '%.4f')
                        clear tmp
                ...
                switch extn
                    case '.dat' %int16 Binary
                        ...
                        if fidOUT > 0 %check if file was created                        
                            ...
                            error(['Could not save ' fullfile(fPath,fName) ...
                                '. Permission denied.'])
                        end
                        fclose(fidOUT);
                    case '.txt' %Text                    
...
%         end
    ...
    function populateFiles(p)
        if (all(p~=0)) && ~isempty(p) && exist(p,'dir')
            ...
            % DATABASE Stuff
           if flagDBtb %if db toolbox exist
                conn = database(dataSource,'','');  %db connection                        
    ...
    %file or mark current file as being "complete".
        ...
        if ~flagDBtb; return; end; %if db toolbox not installed cancel fxn call
        ...
        switch s
            ...
            case 'alt' %if right click, mark file as complete
                ...
                PerOutlier=str2double(PerOutlier(1:end-1)); %remove % sign and convert
                ...
                if ~isempty(answer)
                    recData={fname,date1,true,createdTime,MeanHR,SDHR,PerOutlier, ...
                        TotalOutlier,TotalIBI,outThr,tempThr,answer{2}};
        ...
        try             
            colnames = {'FileName','RecordDate','Complete','CreatedDate','MeanHR', ...
                'SDHR','PerOutlier','TotalOutlier','TotalIBI','OutlierThr', ...
                'TemplateThr','Notes'};
            ...
            'WHERE Complete = 1  AND FileName = ''' fname '''']);
            cursorA = fetch(cursorA);           %fecth records
            close(conn)
            Ann=cursorA.data;                   %only need data            
            ...
            if strcmp(Ann,'No Data') %if file doesn't exist
                conn = database(dataSource,'','');
                fastinsert(conn, 'tblCompletedFiles', colnames, recData) %insert into db
                close(conn) % need to close before repopulating
                populateFiles(path1); %repopulate list                
            else               
                button = questdlg('File is already complete. What do you want to do?', ...
                    'File Alreay Complete','Cancel', ...
